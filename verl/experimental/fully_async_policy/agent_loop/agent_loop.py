...
class _FullyAsyncManagerMixin:
    """Shared methods for fully-async agent loop managers."""

    @auto_await
    async def generate_sequences_single(self, prompts: DataProto) -> DataProto:
        """Dispatch single sample to an agent loop worker.
...
class FullyAsyncAgentLoopManager(_FullyAsyncManagerMixin, AgentLoopManager):
    def __init__(
        self,
        config: DictConfig,
...
@ray.remote
class FullyAsyncPerTurnAgentLoopWorker(PerTurnAgentLoopWorker):
    """Per-turn agent loop worker with fully-async partial-rollout resume capability."""
...
class FullyAsyncPerTurnAgentLoopManager(_FullyAsyncManagerMixin, PerTurnAgentLoopManager):
    """Per-turn agent loop manager for fully-async training."""
    ...
    def __init__(
        ...
        self.agent_loop_workers_class = FullyAsyncPerTurnAgentLoopWorker
        super().__init__(config, worker_group, rollout_resource_pool, teacher_model_manager, reward_loop_worker_handles)
        if self.distillation_enabled:
            raise NotImplementedError("Distillation is not implemented in FullyAsyncPerTurnAgentLoopManager yet.")
